...
void shapshup_main_view_draw(Canvas* canvas, ShapShupMainViewModel* model) {
    ...
    if(model->raw_file != NULL) {
        ...
        format_frequency(model->raw_file->frequency, buffer);
        ...
        snprintf(
            buffer, sizeof(buffer), "%llu %s", model->calc_total_len, model->is_ms ? "ms" : "us");
...
bool shapshup_main_view_input(InputEvent* event, void* context) {
    ...
    if(is_clicked) {
        ...
        if(event->key == InputKeyOk) {
            ...
            if(instance->scale < SCALE_STEP) {
                instance->scale = SCALE_STEP;
            } else if(SHAPSHUP_SCALE_MIN_VALUE < SHAPSHUP_DEFAULT_SCALE_STEP - instance->scale) {
                instance->scale += SCALE_STEP;
            }
...
ShapShupMainView* shapshup_main_view_alloc() {
    ...
    instance->shape_list = malloc(sizeof(ShapShupShapeList));
...
void shapshup_main_view_draw_scale(Canvas* canvas, ShapShupMainViewModel* model) {
    ...
    char format_buffer[FORMAT_BUFFER_SIZE + 1] = {0};
    ...
    snprintf(buffer, BUFFER_SIZE, "%s us", format_buffer);
    ...
    memset(format_buffer, 0, FORMAT_BUFFER_SIZE);
...
void format_number(uint64_t number_to_format, char* output_buffer) {
    int32_t c;
    char buffer[FORMAT_BUFFER_SIZE + 1] = {0};
    char* ptr = NULL;

    snprintf(buffer, sizeof(buffer), "%llu", number_to_format);
    c = 2 - (int32_t)(strlen(buffer) % 3);
...
void format_frequency(uint32_t frequency, char* output_buffer) {
    snprintf(
        output_buffer, FORMAT_BUFFER_SIZE, "%03lu.%03lu MHz", frequency / 1000000 % 1000, frequency / 1000 % 1000);
}
