...
void shapshup_main_view_set_callback(ShapShupMainView *instance,
...
void shapshup_main_view_show_alert(ShapShupMainView *instance, const char *text,
                                   uint32_t ttl) {
  furi_assert(instance);
  instance->alert_dismiss_time = furi_get_tick() + furi_ms_to_ticks(ttl);
  with_view_model(
      instance->view, ShapShupMainViewModel * model,
      {
...
void shapshup_main_view_check_alert(ShapShupMainView *instance) {
  ...
  with_view_model(
      instance->view, ShapShupMainViewModel * model,
      { model->alert_dismiss_time = 0; }, true);
}

void shapshup_main_view_draw_alert_if_needed(Canvas *canvas,
...
void shapshup_main_view_draw(Canvas *canvas, ShapShupMainViewModel *model) {
  ...
  if (model->raw_file != NULL) {
    ...
    snprintf(buffer, sizeof(buffer), "%03ld.%03ld MHz",
    ...
    snprintf(buffer, sizeof(buffer), "%lld %s", model->calc_total_len,
             model->is_ms ? "ms" : "us");

    uint16_t value_width = canvas_string_width(canvas, buffer);
    canvas_draw_str(canvas, width - value_width - 2, STATUS_BAR_INFO_SHIFT,
                    buffer);
    canvas_invert_color(canvas);
    ...
    // canvas_invert_color(canvas);
    // canvas_draw_str_aligned(canvas, 64, 3, AlignCenter, AlignTop,
    // "Shap-Shup"); canvas_invert_color(canvas);
  }
...
bool shapshup_main_view_input(InputEvent *event, void *context) {
  ...
  if (event->key == InputKeyBack && event->type == InputTypeLong) {
    ...
    shapshup_main_view_show_alert(instance, "Press long BACK to EXIT",
                                  SHAPSHUP_ALERT_DEFAULT_TTL);
    return true;
  }
  ...
  if (is_clicked) {
    uint64_t offset_per_page =
        instance->raw_file != NULL
            ? calc_offset_per_page(instance->raw_file->total_len,
                                   instance->raw_file->min_len, instance->scale)
            : 0;
    if (event->key == InputKeyOk) {
      ...
      shapshup_main_view_reset_alert(instance);
      instance->offset = instance->offset < offset_per_page
                             ? 0
                             : instance->offset - offset_per_page;
#ifdef FURI_DEBUG
      FURI_LOG_D(TAG,
                 "L offset: %lld, offset_per_page: %lld, offset: %lld, "
                 "total_len: %lld",
                 instance->offset, offset_per_page, instance->offset,
      ...
      FURI_LOG_D(
          ...
          instance->offset, offset_per_page, calc,
          instance->raw_file->total_len);
#endif
      if (calc < instance->raw_file->total_len) {
        instance->offset = calc;
      } else if (calc > instance->raw_file->total_len &&
      ...
      offset_per_page = instance->raw_file != NULL
      ...
      shapshup_main_view_show_alert(instance, "Zoom-in",
                                    SHAPSHUP_ALERT_DEFAULT_TTL);
      with_view_model(
          instance->view, ShapShupMainViewModel * model,
          {
      ...
      if (instance->scale < SCALE_STEP) {
        instance->scale = SCALE_STEP;
      } else if (SHAPSHUP_DEFAULT_SCALE_STEP - instance->scale >
                 SHAPSHUP_SCALE_MIN_VALUE) {
        instance->scale += SCALE_STEP;
      }
      ...
      shapshup_main_view_show_alert(instance, "Zoom-out",
                                    SHAPSHUP_ALERT_DEFAULT_TTL);
      with_view_model(
          instance->view, ShapShupMainViewModel * model,
          {
...
void shapshup_main_view_enter(void *context) { furi_assert(context); }

void shapshup_main_view_exit(void *context) { furi_assert(context); }
...
ShapShupMainView *shapshup_main_view_alloc() {
  ...
  view_allocate_model(instance->view, ViewModelTypeLocking,
                      sizeof(ShapShupMainViewModel));
  view_set_context(instance->view, instance);
  view_set_draw_callback(instance->view,
                         (ViewDrawCallback)shapshup_main_view_draw);
...
ShapShupFileResults shapshup_main_view_load_file(ShapShupMainView *instance,
                                                 const char *name) {
  furi_assert(instance);
        ...
        instance->view, ShapShupMainViewModel * model,
        {
          ...
          if (model->raw_file != NULL) {
            ...
            if (model->raw_file->total_len > 1000) {
              model->calc_total_len =
                  (uint64_t)model->raw_file->total_len / 1000;
              model->is_ms = true;
            } else {
              model->is_ms = false;
            }
...
void shapshup_main_view_create_shapes(ShapShupMainView *instance,
                                      uint64_t offset_per_page) {
  ...
  for (uint64_t i = 0; i < instance->raw_file->total_count && !last; i++) {
...
#ifdef FURI_DEBUG
    // FURI_LOG_D(TAG, "skip_value: %lld, current_value: %lld", skip_value,
    // current_value);
#endif
    if (current_value > offset_per_page) {
      ...
      // FURI_LOG_I(
      //     TAG,
      //     "current_value > model->offset_per_page: %lld,
      //     model->offset_per_page: %lld", current_value,
      //     model->offset_per_page);
    } else if (current_offset + current_value >
               instance->offset + offset_per_page) {
      last = true;
      ...
      // FURI_LOG_I(
      //     TAG,
      //     "current_offset + current_value > current_offset +
      //     model->offset_per_page: %lld, model->offset_per_page: %lld",
      //     current_value,
      //     model->offset_per_page);
    }
    current_offset += current_value;
    ...
    ShapShupShapeItem *item =
        ShapShupShapeItemArray_push_raw(instance->shape_list->data);
    if (is_negative) {
      item->x = current_x;
...
void shapshup_main_view_draw_scale(Canvas *canvas,
                                   ShapShupMainViewModel *model) {
  char buffer[64];
  ...
  canvas_draw_line(canvas, width - 1, SHAPSHUP_TOP_SCALE, width - 1,
                   SHAPSHUP_TOP_SCALE + 4);
  for (int i = width; i > 0; i -= 16) {
    canvas_draw_line(canvas, i, SHAPSHUP_TOP_SCALE, i, SHAPSHUP_TOP_SCALE + 4);
    canvas_draw_line(canvas, i - 5, SHAPSHUP_TOP_SCALE, i - 5,
                     SHAPSHUP_TOP_SCALE + 2);
    canvas_draw_line(canvas, i - 10, SHAPSHUP_TOP_SCALE, i - 10,
                     SHAPSHUP_TOP_SCALE + 2);
  }

  canvas_draw_line(canvas, 0, SHAPSHUP_TOP_SCALE, width, SHAPSHUP_TOP_SCALE);
  canvas_draw_line(canvas, 0, SHAPSHUP_BOTTOM_SCALE, width,
                   SHAPSHUP_BOTTOM_SCALE);
  ...
  canvas_draw_str(canvas, width - last_value_width,
                  SHAPSHUP_BOTTOM_SCALE + font_height + 1, buffer);
  ...
  //                 // FURI_LOG_I(
  //                 //     TAG,
  //                 //     "continue current_offset: %lld, current_value:
  //                 %lld",
  //                 //     current_offset,
  //                 //     current_value);
  // #endif
  ...
  //             current_value = skip_value + current_value;
  // #ifdef FURI_DEBUG
  //             // FURI_LOG_D(TAG, "skip_value: %lld, current_value: %lld",
  ...
  //                 //     TAG,
  //                 //     "current_value > model->offset_per_page: %lld,
  //                 model->offset_per_page: %lld",
  //                 //     current_value,
  //                 //     model->offset_per_page);
  //             } else if(current_offset + current_value > model->offset +
  //             model->offset_per_page) {
  //                 last = true;
  ...
  //                 //     TAG,
  //                 //     "current_offset + current_value > current_offset +
  //                 model->offset_per_page: %lld, model->offset_per_page:
  //                 %lld",
  //                 //     current_value,
  //                 //     model->offset_per_page);
  //             }
  ...
  //             if(is_negative) {
  //                 canvas_draw_box(canvas, current_x, CHART_LOWEST_POINT - 2,
  //                 current_value + 1, 2);
  //                 // canvas_draw_line(
  //                 //     canvas, current_x, SHAPSHUP_CHART_HIGHTEST_POINT,
  //                 current_x, CHART_LOWEST_POINT);
  //             } else {
  //                 canvas_draw_box(
  //                     canvas,
  //                     current_x,
  ...
  //                 // canvas_draw_line(
  //                 //     canvas, current_x, CHART_LOWEST_POINT, current_x,
  //                 SHAPSHUP_CHART_HIGHTEST_POINT);
  //             }
  //             // canvas_draw_line(canvas, current_x, y, current_x +
  //             current_value, y);
  ...
  //             // FURI_LOG_D(
  //             //     TAG,
  //             //     "current_value: %lld, current_offset: %lld, current_x:
  //             %lld",
  //             //     current_value,
  //             //     current_offset,
  //             //     current_x);
...
bool shapshup_main_view_no_file(ShapShupMainView *instance) {
  return instance->raw_file == NULL ||
         instance->raw_file->result != ShapShupFileResultOk;
}
...
void format_frequency(uint32_t frequency, char *output_buffer) {
  snprintf(output_buffer, 32, "%03ld.%03ld MHz", frequency / 1000000 % 1000,
           frequency / 1000 % 1000);
}
