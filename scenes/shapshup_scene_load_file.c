...
void shapshup_scene_load_file_on_enter(void *context) {
  ...
  dialog_file_browser_set_basic_options(&browser_options, SHAPSHUP_FILE_EXT,
                                        &I_sub1_10px);

  bool res = dialog_file_browser_show(instance->dialogs, file_path, app_folder,
                                      &browser_options);
#endif
  shapshup_show_loading_popup(instance, true);

#ifdef FURI_DEBUG
  FURI_LOG_D(TAG, "file_path: %s, app_folder: %s",
             furi_string_get_cstr(file_path), furi_string_get_cstr(app_folder));
#endif
  if (res) {
    ShapShupFileResults result = shapshup_main_view_load_file(
        instance->view_main, furi_string_get_cstr(file_path));
    ...
    if (result == ShapShupFileResultOk) {
      ...
      dialog_message_set_text(message, furi_string_get_cstr(dialog_msg), 64, 32,
...
#else
      scene_manager_search_and_switch_to_previous_scene(instance->scene_manager,
                                                        ShapshupSceneStart);
#endif
...
void shapshup_scene_load_file_on_exit(void *context) { UNUSED(context); }
