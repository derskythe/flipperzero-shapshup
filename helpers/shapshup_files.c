...
bool stream_seek_to_key_shapshup(Stream *stream, const char *key,
                                 bool strict_mode) {
  bool found = false;
  FuriString *read_key;
...
static bool stream_read_value_shapshup(Stream *stream, FuriString *value,
                                       bool *last) {
  ...
  while (true) {
    ...
    for (uint16_t i = 0; i < was_read; i++) {
      ...
      if (state == LeadingSpace) {
        ...
        if (is_space_shapshup(data)) {
          continue;
        } else if (!stream_seek(stream, i - was_read,
...
ShapShupRawFile *load_file_shapshup(const char *file_path) {
  ...
  do {
    ...
    if (furi_string_cmp_str(temp_str, SUBGHZ_RAW_FILE_TYPE) != 0) {
      ...
      FURI_LOG_E(TAG, "%s, Value: %s",
                 shapshup_files_result_description(instance->result),
                 furi_string_get_cstr(temp_str));
      break;
    }

    if (!flipper_format_read_uint32(fff_data_file, "Frequency", &temp_data32,
                                    1)) {
  ...
  FURI_LOG_I(TAG, "total_count: %lld, total_len: %lld", instance->total_count,
             instance->total_len);
