...
void elements_button_top_left(Canvas *canvas, const char *str) {
  ...
  const uint8_t button_width =
      string_width + horizontal_offset * 2 + icon_width_with_offset;
  ...
  canvas_draw_str(canvas, x + horizontal_offset + icon_width_with_offset,
                  y - vertical_offset, str);
  canvas_invert_color(canvas);
}
...
void elements_button_top_right(Canvas *canvas, const char *str) {
  ...
  canvas_invert_color(canvas);
  canvas_draw_str(canvas, x - button_width + horizontal_offset,
                  y - vertical_offset, str);
  canvas_draw_icon(canvas, x - horizontal_offset - icon_get_width(icon),
